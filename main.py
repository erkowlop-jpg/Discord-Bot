import sys, types, os, random, asyncio, threading
from flask import Flask

# تجاوز مكتبة الصوت في البيئات السحابية
...
# تشغيل خادم الويب في مسار منفصل (Thread)
...
# 2. أمر عرض الأفتار
@bot.tree.command(name="avatar", description="Display a user's avatar in high resolution")
@app_commands.rename(target_user="user")
@app_commands.guild_only()
async def avatar(interaction: discord.Interaction, target_user: discord.Member = None):
    user = target_user or interaction.user
    embed = discord.Embed(
        title=f"🖼️ الصورة الشخصية لـ {user.name}",
        color=EMBED_COLOR
    )
